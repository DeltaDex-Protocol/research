...
def calibrate_sabr(SELECTED_EXPIRY = '29/SEP/23'):
    ...
    options_data['current_time'] = datetime.datetime.now()

    def update_database(df):
        engine = sqlalchemy.create_engine('sqlite:///options_database.db', echo=False)

        df.to_sql('options_table', con=engine, if_exists='append')
    
    update_database(options_data)
    print('db updated', datetime.datetime.now())
